        ...
        if($type!=null){
          ...
          if($pointVente!=null){
           ...
           $qb->addSelect('count(s.id) as nombre'); 
            $qb->addSelect('count(s.absent) as absent'); 
           $qb->addSelect('count(s.enrupture) as enrupture');
...
//situation comparee
  public function findApercuDernier ($region=null, $startDate=null, $endDate=null){
    $em = $this->_em;
    $RAW_QUERY =($region!=null) ?'select s.marque as nom, sum(s.bnreBlle) as bnreblle,count(s.frigo) as frigo,count(s.affiche) as affiche,count(s.affichette) as affichette,count(s.autre) as autre,count(s.potence) as potence,count(s.ncp) as ncp,count(s.tcp) as tcp, count(s.absent) as absent,count(s.enrupture) as enrupture ,count(s.inbar) as inbar,count(s.id) as nombre,max(s.price) as maxprice, avg(s.price) as prixmoyen  from (select v.id,v.date,u.pv from (select pv.id as pv , max(v.date) as date from point_vente pv join rapport v  on pv.id=v.point_vente_id and v.date>=:startDate and v.date<=:endDate and pv.type=:region group by  pv.id order by pv.id) as u  join  rapport v on (u.pv=v.point_vente_id and u.date=v.date) ) as v join situation s on v.id=s.rapport_id  group by s.marque;'  : 'select s.marque as nom, sum(s.bnreBlle) as bnreblle,count(s.frigo) as frigo,count(s.affiche) as affiche,count(s.affichette) as affichette,count(s.autre) as autre,count(s.potence) as potence,count(s.ncp) as ncp,count(s.tcp) as tcp,count(s.inbar) as inbar, count(s.absent) as absent,count(s.enrupture) as enrupture, count(s.id) as nombre,max(s.price) as maxprice, avg(s.price) as prixmoyen  from (select v.id,v.date,u.pv from (select pv.id as pv , max(v.date) as date from point_vente pv join rapport v  on pv.id=v.point_vente_id and v.date>=:startDate and v.date<=:endDate  group by  pv.id order by pv.id) as u  join  rapport v on (u.pv=v.point_vente_id and u.date=v.date) ) as v join situation s on v.id=s.rapport_id  group by s.marque;'; 
